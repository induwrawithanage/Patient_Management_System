...
const PatientProfile = () => {
  ...
  useEffect(() => {
    ...
    if (!token) {
      toast({
        title: "Unauthorized",
        description: "No access token found in local storage.",
        variant: "destructive",
      });
      return;
    }
  
    axios.get("http://localhost:3000/patient/getinformation", {
      headers: {
    ...
    .then((res) => {
      ...
      const fetchedData: PatientData = {
        ...
        nic: user.national_id || "N/A",
        bloodGroup: user.bloodgroup || "N/A",
        sex: user.sex || "N/A",
        weight: params?.weight?.replace(" kg", "") || "",
        height: params?.height?.replace(" cm", "") || "",
        ...
        <CardContent className="p-6 md:p-8 bg-white">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="space-y-6 lg:pl-4">
              ...
              <div>
                <Label htmlFor="contactNo" className="text-sm font-medium text-gray-500">Contact Number</Label>
                {isEditing ? (
                  <Input
                    id="contactNo"
                    value={editData.contactNo}
